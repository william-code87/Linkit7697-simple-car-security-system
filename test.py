...
LabelA=tkinter.Label(Tkwindow,bg='#9F35FF',fg='white',text=f"Speed : 00 cm/s",font=("Helvetica",15,'bold'),width=40,height=10,justify=tkinter.LEFT)
LabelA.pack(side=tkinter.TOP)
buttonStart=tkinter.Button(Tkwindow,anchor=tkinter.S,text="發動",width=10,height=1)
...
time.sleep(1)
for i in range(1,10):
    LabelA.config(text=f"Speed : {i} cm/s")
    LabelA.update_idletasks()
    Tkwindow.update()
    time.sleep(1)
